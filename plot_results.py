...
bfly1 = glob.glob(rundir+'/output_files/bfly_%3d_*.nc'%int(eta))[0]
...
ax1.set_title(r'$\eta$ = %3d km$^2$ s$^{-1}$, V0 = %2.1f m s$^{-1}$'%(int(eta),round(np.max(v1[:,1])*L1*1E3)))
ax1.text(0.01,0.95,r'SFT B$_r$ butterfly diagram',transform=ax1.transAxes,
        fontsize=10,color='brown')

ax2 = plt.subplot(212)
im2 = ax2.pcolormesh(timearr,np.rad2deg(np.arcsin(latbfly)),bflyhmi,cmap='bwr_r',vmax=10,vmin=-10)
...
picklefile = open('./hmi_polar_field_new.p','rb')
...
ax.plot([t_sft[0],t_sft[-1]],[0,0],linewidth=0.5)
...
ax.text(0.02,0.93,r'$\eta$ = %3d km$^2$ s$^{-1}$, V0 = %2.1f m s$^{-1}$'%(int(eta),round(np.max(v1[:,1])*L1*1E3))
...
f1 = glob.glob(rundir+'/output_files/DM_*.dat')[0]
f2 = np.loadtxt(f1)
...
ax1.plot([time[0],time[-1]],[0,0],linewidth=0.5)

ax1.plot(time[:np.where(time >= frac_year('2025-11-08'))[0][0]],f2[:np.where(time >= frac_year('2025-11-08'))[0][0],1],label=r'$\eta$ = %3d km$^2$ s$^{-1}$'%(int(eta)),c='k')
...
vel = glob.glob(rundir+'/output_files/MC_vel*.dat')[0]
v1 = np.loadtxt(vel)
ax2.plot(np.rad2deg(np.arcsin(v1[:,0])),v1[:,1]*L1*1E3,label=f'Peak flow amplitude {round(np.max(v1[:,1]*L1*1E3)):2.1f} m/s',c='r')
...
ax2.set_ylabel('Flow speed [m s$^{-1}$]')
#ax2.minorticks_on()
